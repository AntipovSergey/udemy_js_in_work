...
function cards() {
    ...
    (0,_services_services_js__WEBPACK_IMPORTED_MODULE_0__.getData)('http://localhost:3000/menu')
        .then(data => {
            data.forEach((object) => {
                const {
                    img,
...
function slider({ container, slide, nextArrow, prevArrow, totalCounter, currentCounter, wrapper, field }) {
    ...
    const setDotsOpacity = () => {
        dots.forEach(dot => dot.style.opacity = '.5');
        dots[slideIndex - 1].style.opacity = 1;
    }

    const setCurrentTextContent = () => {
        if (slides.length < 10) {
            current.textContent = `0${slideIndex}`;
        } else {
            current.textContent = slideIndex;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const modalTimerId = setTimeout(() => (0,_modules_modal_js__WEBPACK_IMPORTED_MODULE_4__.showModal)('.modal', modalTimerId), 300000);
    ...
    (0,_modules_modal_js__WEBPACK_IMPORTED_MODULE_4__["default"])('[data-modal]', '.modal', modalTimerId);
    ...
    (0,_modules_timer_js__WEBPACK_IMPORTED_MODULE_6__["default"])('.timer', '2023-5-11');
})
})();
